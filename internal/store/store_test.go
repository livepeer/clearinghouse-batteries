...
func TestMigrationsConstraintsAndRoundTrip(t *testing.T) {
	...
	for _, index := range []string{"allocations_grant", "keys_allocation", "sessions_allocation", "usage_status", "signing_match", "ledger_account", "ledger_transaction", "settlements_match", "settlements_block", "ticket_broker_events_block", "ticket_broker_events_sender"} {
		var n int
		testutil.Must(t, f.DB.DB.QueryRow(`SELECT count(*) FROM sqlite_master WHERE type='index' AND name=?`, index).Scan(&n))
		if n != 1 {
			t.Fatalf("missing index %s", index)
		}
	}
	testutil.Must(t, migrations.Up(ctx, f.DB.DB))
	list, err := migrations.List(ctx, f.DB.DB)
	testutil.Must(t, err)
	if len(list) != 1 || !list[0].Applied {
		t.Fatal(list)
	}
	testutil.Must(t, migrations.Down(ctx, f.DB.DB))
	testutil.Must(t, migrations.Up(ctx, f.DB.DB))
	var count int
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT count(*) FROM grants`).Scan(&count))
	if count != 0 {
		t.Fatal(count)
	}
}
...
func TestEscrowReportUsesArbitraryWidthAmounts(t *testing.T) {
	...
	testutil.Must(t, f.DB.BootstrapChain(ctx, stream, store.Block{Number: -1}, []store.EscrowSnapshot{{ChainID: "42161", Contract: testutil.Contract, Sender: testutil.Sender, Deposit: amount, Reserve: amount}}))
...
func TestEscrowSnapshotIsAHardReorgBoundary(t *testing.T) {
	...
	testutil.Must(t, f.DB.BootstrapChain(ctx, stream, store.Block{Number: 5, Hash: hash}, []store.EscrowSnapshot{{ChainID: "42161", Contract: testutil.Contract, Sender: testutil.Sender, Deposit: "1", Reserve: "2"}}))
...
func TestIngestExactMoneyReplayQuarantineOverdraw(t *testing.T) {
	...
	testutil.Must(t, f.DB.Ingest(ctx, "test", 0, raw))
	testutil.Must(t, f.DB.Ingest(ctx, "test", 0, raw))
	testutil.Must(t, f.DB.Ingest(ctx, "test", 1, raw))
	testutil.Must(t, f.DB.Ingest(ctx, "test", 2, []byte(`{bad`)))
	testutil.Must(t, f.DB.Ingest(ctx, "test", 3, f.Event(t, "event-2", "17", testutil.PM)))
	...
	var signed int
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT count(*) FROM signing_authorizations`).Scan(&signed))
	if signed != 2 {
		t.Fatal(signed)
	}
	...
	testutil.Must(t, err)
	defer db.Close()
	testutil.Must(t, db.Ingest(ctx, "test", 3, f.Event(t, "event-2", "17", testutil.PM)))
	assertBalanced(t, f.DB)
}
...
func TestQuarantineBindingsAndConflictingID(t *testing.T) {
	...
	testutil.Must(t, f.DB.Ingest(ctx, "test", 0, valid))
	...
	for i, raw := range variants {
		testutil.Must(t, f.DB.Ingest(ctx, "test", int64(i+1), raw))
	}
	var quarantined int
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT count(*) FROM usage_events WHERE status='quarantined'`).Scan(&quarantined))
	if quarantined != 4 {
		t.Fatal(quarantined)
	}
	bal, err := store.Balance(ctx, f.DB.DB, "allocation_available", f.Allocation)
...
func TestConcurrentFundingCannotOverallocate(t *testing.T) {
	f := testutil.New(t, "100")
	ctx := context.Background()
	testutil.Must(t, f.DB.Fund(ctx, "grant", f.Grant, "100"))
...
func TestAllAllocationCreationAndFunding(t *testing.T) {
	...
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT allocated_wei,status FROM grant_allocations WHERE id=?`, zeroAllocation).Scan(&allocated, &status))
	if allocated != "0" || status != "exhausted" {
		t.Fatal(allocated, status)
	}

	testutil.Must(t, f.DB.Fund(ctx, "grant", f.Grant, "75"))
	allocation, err := f.DB.Create(ctx, "allocation", store.Create{Name: "everything", GrantID: f.Grant, Amount: "all"})
	testutil.Must(t, err)
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT allocated_wei,status FROM grant_allocations WHERE id=?`, allocation).Scan(&allocated, &status))
	...
	testutil.Must(t, f.DB.Fund(ctx, "grant", f.Grant, "25"))
	testutil.Must(t, f.DB.Fund(ctx, "allocation", allocation, "all"))
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT allocated_wei FROM grant_allocations WHERE id=?`, allocation).Scan(&allocated))
	if allocated != "100" {
		t.Fatal(allocated)
	}
	assertBalanced(t, f.DB)
...
func TestConcurrentAllUsesOneTransactionalBalance(t *testing.T) {
	...
	testutil.Must(t, f.DB.Fund(ctx, "grant", f.Grant, "60"))
...
func TestCreateKeyForGrantIsAtomicAndUsesAllocationDefaults(t *testing.T) {
	f := testutil.New(t, "100")
	ctx := context.Background()
	testutil.Must(t, f.DB.Fund(ctx, "grant", f.Grant, "50"))
	var before int
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT count(*) FROM grant_allocations`).Scan(&before))
	...
	var after int
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT count(*) FROM grant_allocations`).Scan(&after))
	if after != before {
	...
	var starts, ends any
	testutil.Must(t, f.DB.DB.QueryRow(`SELECT name,beneficiary,allocated_wei,metadata,starts_at_ms,ends_at_ms,status FROM grant_allocations WHERE id=?`, allocation).Scan(&name, &beneficiary, &amount, &metadata, &starts, &ends, &allocationStatus))
...
func TestRevocationReturnsOnlyUnspentAndLateChargeIsRecorded(t *testing.T) {
	f := testutil.New(t, "100")
	ctx := context.Background()
	testutil.Must(t, f.DB.Ingest(ctx, "test", 0, f.Event(t, "one", "40", testutil.PM)))
	testutil.Must(t, f.DB.SetStatus(ctx, "allocation", f.Allocation, "revoked"))
	testutil.Must(t, f.DB.SetStatus(ctx, "allocation", f.Allocation, "revoked"))
	testutil.Must(t, f.DB.Ingest(ctx, "test", 1, f.Event(t, "late", "20", testutil.PM)))
	bal, err := store.Balance(ctx, f.DB.DB, "allocation_available", f.Allocation)
	testutil.Must(t, err)
	if bal.String() != "-20" {
		t.Fatal(bal)
...
func assertBalanced(t *testing.T, db *store.Store) {
	...
	for rows.Next() {
		var id string
		var n, a, d int
		testutil.Must(t, rows.Scan(&id, &n, &a, &d))
		if n != 2 || a != 1 || d != 2 {
			t.Fatalf("unbalanced %s", id)
		}
	}
	testutil.Must(t, rows.Err())
}
