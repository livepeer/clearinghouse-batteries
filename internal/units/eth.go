// Package units converts between user-facing ETH decimals and exact wei integers.
package units
...
func ETHToWei(value string) (string, error) {
	if value == "" {
		return "", errors.New("amount in ETH is required")
	}
	parts := strings.Split(value, ".")
	if len(parts) > 2 || parts[0] == "" || (len(parts) == 2 && parts[1] == "") {
		return "", errors.New("amount must be an unsigned decimal ETH value")
	}
	...
	if len(digitsValue) > 256 {
		return "", errors.New("amount exceeds the 256-digit wei limit")
	}
...
func WeiToETH(value string) (string, error) {
	...
	whole.QuoRem(n, weiPerETH, remainder)
	out := whole.String()
	if remainder.Sign() != 0 {
